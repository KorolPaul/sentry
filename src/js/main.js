...
const appScreensSlider = document.querySelectorAll('.app_screens-wrapper:not(.js-photo-gallery)');
...
const photoGallerySlider = document.querySelectorAll('.js-photo-gallery');
...
themesButtons.forEach(el => {
  ...
  el.addEventListener('click', (e) => {
    ...
    document.querySelectorAll('.themes_image, .themes_content').forEach(el => {
      if (el !== e.target) {
        el.classList.remove('active')
      }
    });
    document.querySelector(`.themes_image[data-tab="${tab}"], .themes_content[data-tab="${tab}"]`).classList.add('active');
  });
});
